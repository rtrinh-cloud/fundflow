...
const store = {
  async get(key: string) {
    try { const r = await (window as any).storage.get(key); return r ? JSON.parse(r.value) : null; } catch { return null; }
  },
...
export default function FundFlow() {
  ...
  const [investorAccount, setInvestorAccount] = useState<any>(null);
  const [founderAccount, setFounderAccount] = useState<any>(null);
  ...
  const [authForm, setAuthForm] = useState<any>({ name:"", firm:"", email:"", password:"", schedulingLink:"", focusVerticals:[], focusStages:[] });
  const [loginForm, setLoginForm] = useState<any>({ email:"", password:"" });
  const [founderForm, setFounderForm] = useState<any>({ name:"", company:"", logoDataUrl:"", vertical:"SaaS", stage:"Seed", roundAmount:"", description:"", fundraising:true, email:"", password:"", schedulingLink:"", deckUrl:"" });
  const [contactForm, setContactForm] = useState<any>({ message:"" });
  ...
  const uploadDeck = async (file: File): Promise<string | null> => {
    ...
    try {
      ...
      return `${SUPABASE_URL}/storage/v1/object/public/decks/${fileName}`;
    } catch { return null; }
    finally { setDeckUploading(false); }
  };

  const scoreMatches = async (cos: any[], investor: any) => {
...
Startups:
${cos.filter((c: any) => c.fundraising).map((c: any, i: number) => `${i + 1}. id="${c.id}" | ${c.name} | ${c.vertical} | ${c.stage} | ${c.roundAmount} | "${c.description}"`).join("\n")}
Return ONLY a JSON array, no markdown. Each item: {"id":"...","score":85,"reason":"One sentence"}. Score 0-100.`;
      ...
      setCompanies((prev: any[]) => prev.map((c: any) => { const sc = scores.find((x: any) => x.id === c.id); return sc ? { ...c, matchScore: sc.score, matchReason: sc.reason } : c; }));
  ...
  const handleInvestorSignup = async () => {
    ...
    const acc: any = { id: `inv-${Date.now()}`, ...authForm, createdAt: new Date().toISOString() };
    const updated: any[] = [...investors, acc];
  ...
  const handleInvestorLogin = async () => {
    ...
    const acc = investors.find((i: any) => i.email.toLowerCase() === loginForm.email.toLowerCase() && i.password === loginForm.password);
  ...
  const handleFounderLogin = async () => {
    setAuthError("");
    if (!loginForm.email || !loginForm.password) return setAuthError("Please enter your email and password.");
    const co = companies.find((c: any) => c.founderEmail.toLowerCase() === loginForm.email.toLowerCase() && c.password === loginForm.password);
  ...
  const handleRecovery = () => {
    ...
    const account: any = role === "investor"
      ? investors.find((i: any) => i.email.toLowerCase() === loginForm.email.toLowerCase())
      : companies.find((c: any) => c.founderEmail.toLowerCase() === loginForm.email.toLowerCase());
    if (!account) return setAuthError("No account found with that email.");
    setRecoveredPassword(account.password);
  };

  const handleFounderSubmit = async () => {
    setFounderError("");
    if (!founderForm.name || !founderForm.company || !founderForm.email || !founderForm.password)
      return setFounderError("Please fill all required fields.");
    if (companies.find((c: any) => c.founderEmail.toLowerCase() === founderForm.email.toLowerCase()))
    ...
    const co: any = {
      ...
      deckUrl: founderForm.deckUrl, bookmarked: false, matchScore: null, matchReason: "",
  ...
  const toggleBookmark = async (id: string) => {
    const updated = companies.map((c: any) => c.id === id ? { ...c, bookmarked: !c.bookmarked } : c);
    await saveCompanies(updated);
  };
  ...
  const openModal = (type: string) => { setAuthError(""); setRecoveredPassword(""); setLoginForm({ email: "", password: "" }); setAuthForm({ name: "", firm: "", email: "", password: "", schedulingLink: "", focusVerticals: [], focusStages: [] }); setAuthModal(type); };
  ...
  const scoreColor = (s: number) => s >= 80 ? "#00f5a0" : s >= 60 ? "#f5c400" : "#ff6b6b";

  const Logo = ({ url, name, size = 44 }: { url: string; name: string; size?: number }) =>
    url ? <img src={url} alt={name} style={{ width: size, height: size, borderRadius: 10, objectFit: "cover", border: `1px solid ${BORDER}`, flexShrink: 0 }} />
  ...
  const CheckList = ({ items, selected, onChange }: { items: string[]; selected: string[]; onChange: (v: string[]) => void }) => (
  ...
  const renderModal = () => {
    ...
    if (authModal === "recover-investor" || authModal === "recover-founder") return (
      <div style={s.modal} onClick={() => { setAuthModal(null); setRecoveredPassword(""); }}>
        <div style={s.modalBox} onClick={(e: any) => e.stopPropagation()}>
          <h2 style={{ margin: "0 0 4px", fontWeight: 800 }}>Recover Password</h2>
          <p style={{ color: "#888", fontSize: 13, marginTop: 0, marginBottom: 20 }}>Enter your email and we will show your password.</p>
          ...
          {recoveredPassword ? (
            <div>
              <div style={{ background: "#001a0f", border: `1px solid #00f5a044`, borderRadius: 10, padding: 16, marginBottom: 16, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 800, color: ACCENT }}>{recoveredPassword}</div>
              </div>
            ...
            <div>
              ...
              <input style={s.input} placeholder="you@company.com" value={loginForm.email} onChange={(e: any) => setLoginForm((f: any) => ({ ...f, email: e.target.value }))} />
    ...
    if (authModal === "investor-login") return (
      <div style={s.modal} onClick={() => setAuthModal(null)}>
        <div style={s.modalBox} onClick={(e: any) => e.stopPropagation()}>
          ...
          {authError && <div style={s.error}>{authError}</div>}
          <label style={s.label}>Email *</label>
          <input style={s.input} placeholder="jane@sequoia.com" value={loginForm.email} onChange={(e: any) => setLoginForm((f: any) => ({ ...f, email: e.target.value }))} />
          <label style={s.label}>Password *</label>
          <input style={s.input} type="password" placeholder="••••••••" value={loginForm.password} onChange={(e: any) => setLoginForm((f: any) => ({ ...f, password: e.target.value }))} />
          ...
          <div style={s.divider}><div style={s.dividerLine} /><span style={s.dividerText}>no account yet?</span><div style={s.dividerLine} /></div>
    ...
    if (authModal === "investor-signup") return (
      <div style={s.modal} onClick={() => setAuthModal(null)}>
        <div style={s.modalBox} onClick={(e: any) => e.stopPropagation()}>
          ...
          <input style={s.input} placeholder="Jane Smith" value={authForm.name} onChange={(e: any) => setAuthForm((f: any) => ({ ...f, name: e.target.value }))} />
          <label style={s.label}>Firm Name</label>
          <input style={s.input} placeholder="Sequoia Capital" value={authForm.firm} onChange={(e: any) => setAuthForm((f: any) => ({ ...f, firm: e.target.value }))} />
          <label style={s.label}>Email *</label>
          <input style={s.input} placeholder="jane@sequoia.com" value={authForm.email} onChange={(e: any) => setAuthForm((f: any) => ({ ...f, email: e.target.value }))} />
          <label style={s.label}>Scheduling Link</label>
          <input style={s.input} placeholder="https://calendly.com/jane" value={authForm.schedulingLink} onChange={(e: any) => setAuthForm((f: any) => ({ ...f, schedulingLink: e.target.value }))} />
          <label style={s.label}>Focus Verticals</label>
          <CheckList items={VERTICALS} selected={authForm.focusVerticals} onChange={(v: string[]) => setAuthForm((f: any) => ({ ...f, focusVerticals: v }))} />
          <label style={s.label}>Focus Stages</label>
          <CheckList items={STAGES} selected={authForm.focusStages} onChange={(v: string[]) => setAuthForm((f: any) => ({ ...f, focusStages: v }))} />
          <label style={s.label}>Password *</label>
          <input style={s.input} type="password" placeholder="••••••••" value={authForm.password} onChange={(e: any) => setAuthForm((f: any) => ({ ...f, password: e.target.value }))} />
  ...
  if (view === "home") return (
    <div style={s.app}>
      ...
      <div style={{ ...s.page, textAlign: "center", paddingTop: 100 }}>
        ...
        <p style={s.sub}>Post your company. Investors discover, get AI-matched, and connect.</p>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 28 }}>
          <button style={s.btn} onClick={() => setView("founder")}>I am a Founder</button>
          <button style={s.btnOutline} onClick={() => openModal("investor-signup")}>I am an Investor</button>
        </div>
      </div>
      {renderModal()}
  ...
  if (view === "founder") return (
    <div style={s.app}>
      ...
      <div style={{ ...s.page, maxWidth: 600 }}>
        ...
        <p style={s.sub}>Fill in your details to be discovered by investors.</p>
        <div style={{ ...s.card, marginTop: 32 }}>
          <label style={s.label}>Founder Name *</label>
          <input style={s.input} placeholder="Alex Johnson" value={founderForm.name} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, name: e.target.value }))} />
          <label style={s.label}>Company Name *</label>
          <input style={s.input} placeholder="AcmeCorp" value={founderForm.company} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, company: e.target.value }))} />
          ...
          <div style={{ marginBottom: 12 }}>
            {founderForm.logoDataUrl && <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              ...
              <button style={{ ...s.btnOutline, fontSize: 12, padding: "6px 12px" }} onClick={() => { setFounderForm((f: any) => ({ ...f, logoDataUrl: "" })); if (logoRef.current) logoRef.current.value = ""; }}>Remove</button>
            </div>}
            <input ref={logoRef} type="file" accept="image/jpeg,image/png,image/jpg,image/webp" style={{ ...s.input, padding: "8px 14px", cursor: "pointer" }} onChange={async (e: any) => { const file = e.target.files[0]; if (!file) return; const r = new FileReader(); r.onload = () => setFounderForm((f: any) => ({ ...f, logoDataUrl: r.result })); r.readAsDataURL(file); }} />
          </div>
          <label style={s.label}>Vertical</label>
          <select style={s.select} value={founderForm.vertical} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, vertical: e.target.value }))}>{VERTICALS.map((v: string) => <option key={v}>{v}</option>)}</select>
          <label style={s.label}>Stage</label>
          <select style={s.select} value={founderForm.stage} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, stage: e.target.value }))}>{STAGES.map((st: string) => <option key={st}>{st}</option>)}</select>
          <label style={s.label}>Round Amount</label>
          <input style={s.input} placeholder="e.g. $2M" value={founderForm.roundAmount} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, roundAmount: e.target.value }))} />
          <label style={s.label}>Short Description</label>
          <textarea style={{ ...s.input, height: 80, resize: "vertical" }} placeholder="We build..." value={founderForm.description} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, description: e.target.value }))} />
          <label style={s.label}>Pitch Deck (PDF or PPTX)</label>
          <div style={{ marginBottom: 12 }}>
            {founderForm.deckUrl && <div style={{ background: "#001a0f", border: `1px solid #00f5a044`, borderRadius: 8, padding: "8px 12px", fontSize: 13, color: ACCENT, marginBottom: 8 }}>Deck uploaded</div>}
            <input ref={deckRef} type="file" accept=".pdf,.pptx,.ppt" style={{ ...s.input, padding: "8px 14px", cursor: "pointer" }} onChange={async (e: any) => {
              const file = e.target.files[0];
              if (!file) return;
              const url = await uploadDeck(file);
              if (url) setFounderForm((f: any) => ({ ...f, deckUrl: url }));
              else showToast("Deck upload failed. Check Supabase storage setup.");
            }} />
            {deckUploading && <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>Uploading...</div>}
          </div>
          <label style={s.label}>Your Email *</label>
          <input style={s.input} placeholder="you@company.com" value={founderForm.email} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, email: e.target.value }))} />
          <label style={s.label}>Scheduling Link</label>
          <input style={s.input} placeholder="https://calendly.com/yourname" value={founderForm.schedulingLink} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, schedulingLink: e.target.value }))} />
          <label style={s.label}>Password *</label>
          <input style={s.input} type="password" placeholder="••••••••" value={founderForm.password} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, password: e.target.value }))} />
          <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 16 }}>
            <input type="checkbox" id="fr" checked={founderForm.fundraising} onChange={(e: any) => setFounderForm((f: any) => ({ ...f, fundraising: e.target.checked }))} style={{ accentColor: ACCENT, width: 16, height: 16 }} />
  ...
  if (view === "founder-dashboard" && founderAccount) return (
    <div style={s.app}>
      <NavBar right={<><span style={{ fontSize: 13, color: "#888", marginRight: 8 }}>Hi {founderAccount.founderName}</span><button style={s.navBtn} onClick={signOutFounder}>Sign Out</button></>} />
      <div style={{ ...s.page, maxWidth: 600 }}>
        ...
        <div style={{ ...s.card, marginTop: 24 }}>
          {[["Vertical", founderAccount.vertical], ["Stage", founderAccount.stage], ["Round", founderAccount.roundAmount], ["Email", founderAccount.founderEmail]].map(([k, v]: any) => (
      ...
      <div style={s.page}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16, marginBottom: 32 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <select style={{ ...s.select, width: "auto", marginBottom: 0 }} value={filterVertical} onChange={(e: any) => setFilterVertical(e.target.value)}>
              <option value="All">All Verticals</option>{VERTICALS.map((v: string) => <option key={v}>{v}</option>)}
            </select>
            <select style={{ ...s.select, width: "auto", marginBottom: 0 }} value={filterStage} onChange={(e: any) => setFilterStage(e.target.value)}>
              <option value="All">All Stages</option>{STAGES.map((st: string) => <option key={st}>{st}</option>)}
            </select>
            <button style={{ ...(showSaved ? s.btn : s.btnOutline), padding: "10px 16px" }} onClick={() => setShowSaved((v: boolean) => !v)}>{showSaved ? "Saved" : "Saved"}</button>
          </div>
        </div>
            ...
            <div key={co.id} style={s.companyCard}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                  ...
                  {co.matchScore !== null && co.matchScore !== undefined && <div style={{ background: "#111", border: `1px solid ${scoreColor(co.matchScore)}44`, borderRadius: 8, padding: "3px 8px", textAlign: "center" }}><div style={{ fontSize: 15, fontWeight: 800, color: scoreColor(co.matchScore) }}>{co.matchScore}%</div><div style={{ fontSize: 9, color: "#666", letterSpacing: "0.5px" }}>MATCH</div></div>}
                  {(co.matchScore === null || co.matchScore === undefined) && loadingMatch && <div style={{ fontSize: 11, color: "#555" }}>scoring...</div>}
                </div>
              </div>
      ...
      {contactModal && (
        <div style={s.modal} onClick={() => setContactModal(null)}>
          <div style={s.modalBox} onClick={(e: any) => e.stopPropagation()}>
            ...
            <textarea style={{ ...s.input, height: 90, resize: "vertical" }} placeholder={`Hi ${contactModal.founderName}, I would love to learn more...`} value={contactForm.message} onChange={(e: any) => setContactForm((f: any) => ({ ...f, message: e.target.value }))} />
            {contactModal.schedulingLink ? <div style={{ background: "#001a0f", border: `1px solid #00f5a022`, borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 13, color: ACCENT }}>Founder scheduling link will be included.</div> : <div style={{ background: "#1a1500", border: "1px solid #f5c50022", borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 13, color: "#f5c500" }}>Founder has not added a scheduling link.</div>}
