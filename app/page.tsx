...
Return ONLY a JSON array, no markdown. Each item: {"id":"...","score":85,"reason":"One sentence"}. Score 0-100.`;
  ...
  const handleRecovery = () => {
    ...
    if (role === "investor") account = investors.find((i:any)=>i.email.toLowerCase()===loginForm.email.toLowerCase());
    else account = companies.find((c:any)=>c.founderEmail.toLowerCase()===loginForm.email.toLowerCase());
  ...
  const Logo = ({url, name, size=44}: {url:string, name:string, size?:number}) => url
  ...
  const CheckList = ({items, selected, onChange}: {items:string[], selected:string[], onChange:(v:string[])=>void}) => (
    <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12}}>
      {items.map(i=>{const on=selected.includes(i);return <button key={i} onClick={()=>onChange(on?selected.filter((x:string)=>x!==i):[...selected,i])} style={{background:on?"#001a0f":"#1a1a1a",border:`1px solid ${on?ACCENT+"66":BORDER}`,color:on?ACCENT:"#aaa",padding:"5px 12px",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:on?700:400}}>{i}</button>;})}
    </div>
  );
        ...
        <div style={s.modalBox} onClick={(e:any)=>e.stopPropagation()}>
          ...
          {recoveredPassword ? (
            ...
            <div>
              ...
              <input style={s.input} placeholder="you@company.com" value={loginForm.email} onChange={(e:any)=>setLoginForm((f:any)=>({...f,email:e.target.value}))} autoFocus/>
      ...
      {contactModal && (
        <div style={s.modal} onClick={()=>setContactModal(null)}>
          <div style={s.modalBox} onClick={(e:any)=>e.stopPropagation()}>
            ...
            <textarea style={{...s.input,height:90,resize:"vertical"}} placeholder={`Hi ${contactModal.founderName}, I'd love to learn more…`} value={contactForm.message} onChange={(e:any)=>setContactForm((f:any)=>({...f,message:e.target.value}))}/>
